...
if ( ! function_exists( 'boldthemes_search_form' ) ) {
	function boldthemes_search_form( $form ) {
		...
		$form .= boldthemes_get_icon_html( array( 'icon' => 'fa_f002', 'url' => '#', 'url_title' => esc_attr__( 'Search', 'cliniq' ) ) );
		$form .= '
		<div class="btSearchInner gutter" role="search">
			<div class="btSearchInnerContent port">
				<form action="' . esc_attr( home_url( '/' ) ) . '" method="get"><input type="text" name="s" placeholder="' . esc_attr__( 'Looking for...', 'cliniq' ) . '" class="untouched">
...
if ( ! function_exists( 'boldthemes_override_unload_textdomain' ) ) {
	function boldthemes_override_unload_textdomain( $override, $domain ) {
		if ( $domain === 'cliniq' || $domain === 'bt_plugin' ) {
